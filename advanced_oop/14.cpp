...
class Podcast{
    ...
    friend ostream& operator<<(ostream & out, Podcast & p){
        out << p.name << " " << p.url << " " << p.cost << " " << p.discount << endl;
    }
...
public:
    ...
    friend ostream& operator<<(ostream & out, StreamingPodcast & sp){
        out << sp.monthlyFee << " " << sp.month << " " << sp.year << endl;
        return out;
    }
};
