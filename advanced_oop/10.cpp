...
private:
    ...
    int m;
    double udel;
    double provision_percent;
    double entry_provision;
    void copy(const ActionFund & af){
        strcpy(this->name, af.name);
        m = af.m;
        udel = af.udel;
        provision_percent = af.provision_percent;
        entry_provision = af.entry_provision;
...
public:
    ...
    ActionFund(char * name, int m, double udel) {
        strcpy(this->name, name);
        this->m = m;
        this->udel = udel;
        investmentFund = new InvestmentFund[0];
    }
